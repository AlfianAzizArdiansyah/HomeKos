    ...
    <div class="w-full max-w-6xl mx-auto mt-12 px-4">
        <div class="bg-white rounded-lg shadow-md p-6 space-y-6">
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <h2 class="text-2xl font-semibold text-gray-800">Riwayat Pembayaran</h2>
            ...
            <!-- Table Section -->
            <div>
                <h3 class="text-xl font-semibold text-gray-700 mb-4">Riwayat Bayar Bulanan</h3>
                <div class="overflow-x-auto shadow rounded-lg">
                    <table class="w-full text-sm md:text-base text-center border-collapse bg-white text-gray-800">
                            ...
                            @forelse($riwayatPembayaran as $data)
                                <tr>
                                    <td class="py-2 px-4 border">
                                        {{ \Carbon\Carbon::parse($data->tanggal_bayar)->translatedFormat('F Y') }}</td>
                                    <td class="py-2 px-4 border">
                                        {{ \Carbon\Carbon::parse($data->tanggal_bayar)->format('d M Y') }}</td>
                                    <td class="py-2 px-4 border">Rp {{ number_format($data->jumlah, 0, ',', '.') }}</td>
                                    <td class="py-2 px-4 border">
                                        @if ($data->bukti_bayar)
                                    ...
                                    <td class="py-2 px-4 border">
                                        <a href="{{ route('penghuni.pembayaran.invoice', $data->id) }}" target="_blank"
                                            class="bg-green-500 hover:bg-green-600 text-white text-xs md:text-sm px-3 py-1 rounded-md shadow inline-flex items-center justify-center"
