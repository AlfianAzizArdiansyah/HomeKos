                                    ...
                                    <tr class="border-t hover:bg-blue-50 text-md">
                                        ...
                                        <td class="px-6 py-4">
                                            <div class="flex justify-center items-center gap-2">
                                                <button @click="openEditPembayaran({id: {{ $pembayaran->id }},jumlah: {{ $pembayaran->jumlah }},jatuh_tempo: '{{ \Carbon\Carbon::parse($pembayaran->jatuh_tempo)->format('Y-m-d') }}',tanggal_bayar: '{{ $pembayaran->tanggal_bayar ? \Carbon\Carbon::parse($pembayaran->tanggal_bayar)->format('Y-m-d') : '' }}',status: '{{ $pembayaran->status }}'})" class="bg-indigo-600 hover:bg-indigo-700 text-white text-sm px-5 py-2 rounded-md shadow">
                                                    Edit
                                                </button>
