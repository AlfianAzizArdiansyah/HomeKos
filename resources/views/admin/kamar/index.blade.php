@extends('layouts.app')
@section('content')
    <div x-data="{ tambahKamar: false, editKamar: false, kamarData: {} }" class="p-4 md:p-6  bg-white rounded-xl shadow-md">

        <!-- Judul & Tombol -->
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 md:mb-8 gap-4">
            <h1 class="text-3xl md:text-4xl font-extrabold text-gray-800 tracking-wide">Manajemen Kamar</h1>
        ...
        <div class="overflow-x-auto rounded-lg shadow">
            <table class="w-full border-collapse bg-white text-gray-800 text-sm md:text-lg">
                            ...
                            <td class="px-6 py-4 space-x-1">
                                @php
                                    $fasilitas = is_array($kamar->fasilitas)
                                        ? $kamar->fasilitas
                                        : json_decode($kamar->fasilitas, true);
                                @endphp
                                ...
                                @if (!empty($fasilitas))
                                    ...
                                    @endforeach
                                @else
                                    <span class="inline-block text-gray-500 italic text-sm">Kamar Kosongan</span>
                                @endif
                            </td>
